...
class AXLightSummarizer:
    ...
    def __init__(self, model_path: str = r"models\A.X-4.0-Light"):
        ...
        # Step 6: 문장 완성 우선 (min_tokens 유연화)
        self.min_generation_tokens = 100  # 유연성 향상으로 자연스러운 문장 완성
        self.max_generation_tokens = 280  # Qwen3와 동일
        ...
        self.generation_params = {
            ...
            'early_stopping': True,  # 문장 완성까지 생성
            'use_cache': True
        }
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if hasattr(self.model, 'generation_config') and self.model.generation_config:
                ...
                gen_conf.top_p = None
                gen_conf.top_k = None
                gen_conf.early_stopping = True  # 문장 완성 보장
    ...
    def build_efficient_korean_prompt(self, conversation_text: str) -> str:
        ...
        system_prompt = (
            ...
            "출력 형식 (반드시 아래 순서대로 한 번만 작성):\\n"
            "첫 번째 줄: **고객**: 고객의 핵심 질문이나 요청사항\\n"
            "두 번째 줄: **상담사**: 상담사의 핵심 안내나 해결방안\\n"
            "세 번째 줄: **상담결과**: 상담의 최종 결과나 처리상태\\n\\n"
            "⚠️ 중요: 각 섹션은 단 한 번만 등장해야 하며, 다른 섹션 내부에 포함되어선 안 됩니다.\\n"
    ...
    def generate_summary_fast(self, conversation_text: str) -> Tuple[str, float]:
        ...
        try:
            ...
            # 항상 최대 토큰 사용 (문장 완성 보장)
            generation_kwargs['max_new_tokens'] = self.max_generation_tokens
            generation_kwargs['min_new_tokens'] = self.min_generation_tokens
