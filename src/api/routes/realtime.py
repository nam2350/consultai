...
Realtime 모델을 사용한 빠른 상담 요약 제공 (1-3초 목표)
...
from ...api.dependencies.auth import verify_realtime_permission

# 라우터 생성
router = APIRouter(prefix="/consultation", tags=["실시간 상담 분석 (Realtime)"])

# Realtime 서비스 인스턴스 (지연 초기화)
_realtime_service = None
_realtime_model_type = "qwen3_1_7b"  # 기본: Qwen3-1.7B (2.83초, 0.800 품질)
...
async def get_realtime_service():
    ...
    if _realtime_service is None:
        logger.info("[실시간 API] Realtime 서비스 초기화 시작...")
        ...
        try:
            ...
            if not success:
                logger.error("[실시간 API] Realtime 모델 로드 실패")
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                    detail="Realtime 모델 초기화 실패"
                )

            logger.info(f"[실시간 API] Realtime 모델 로드 완료: Qwen3-1.7B")

        except Exception as e:
            logger.error(f"[실시간 API] Realtime 서비스 초기화 오류: {e}")
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
async def realtime_analyze(
    ...
    key_info: Dict[str, Any] = Depends(verify_realtime_permission)
) -> RealtimeAnalysisResponse:
    """
    실시간 상담 분석 API
    ...
    try:
        ...
        # 4. 빠른 요약 생성
        logger.info(f"[실시간 API] Realtime 요약 시작 - 대화 길이: {len(conversation_text)}자")
...
@router.get(
    ...
    description="Realtime 모델 로드 상태 및 처리 통계 조회"
)
async def get_realtime_status():
    ...
    return {
        "status": "healthy" if _realtime_service is not None else "not_initialized",
        "model_loaded": _realtime_service is not None,
        "model_name": "Qwen3-1.7B" if _realtime_service else None,
        "model_type": "Realtime (Fast Summary Model)",
